...
export class SleeperProvider extends BaseFantasyProvider {
  ...
  private transformSleeperPlayers(sleeperData: any): PlayersMap {
    ...
    Object.entries(sleeperData).forEach(
      ([playerId, playerData]: [string, any]) => {
        players[playerId] = {
          ...
          sportradar_id:
            playerData.sportradar_id || playerData.metadata?.sportradar_id,
          ...
          rotowire_id:
            playerData.rotowire_id || playerData.metadata?.rotowire_id,
          fantasypros_id:
            playerData.fantasypros_id || playerData.metadata?.fantasypros_id,
  ...
  private transformSleeperStats(sleeperData: any): StatsMap {
    ...
    Object.entries(sleeperData).forEach(
      ([playerId, playerStats]: [string, any]) => {
        ...
        if (playerStats.pts_half_ppr !== undefined)
          points.half_ppr = playerStats.pts_half_ppr;
        if (playerStats.pts_std !== undefined)
          points.standard = playerStats.pts_std;
  ...
  private transformSleeperProjections(sleeperData: any): ProjectionsMap {
    ...
    Object.entries(sleeperData).forEach(
      ([playerId, playerProjection]: [string, any]) => {
        ...
        // Extract points if available
        const points: any = {};
        if (playerProjection.pts_ppr !== undefined)
          points.ppr = playerProjection.pts_ppr;
        if (playerProjection.pts_half_ppr !== undefined)
          points.half_ppr = playerProjection.pts_half_ppr;
        if (playerProjection.pts_std !== undefined)
          points.standard = playerProjection.pts_std;
