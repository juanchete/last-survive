...
-- Add MVP columns to fantasy_teams table
ALTER TABLE fantasy_teams 
ADD COLUMN IF NOT EXISTS mvp_wins INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS total_earnings NUMERIC DEFAULT 0;

-- Create weekly_mvp_history table
CREATE TABLE IF NOT EXISTS weekly_mvp_history (
  ...
  earnings NUMERIC DEFAULT 20.00,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  UNIQUE(league_id, week, season)
);

-- Create function to process weekly MVP
...
-- Update the weekly elimination function to also process MVP
